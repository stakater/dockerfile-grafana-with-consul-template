...
# Grafana container (with the alias consul, of course). But this environment variable can also be overridden when we run the
...
# our container will expose ports, where Grafana will be running

EXPOSE     3000

# Make daemon service dir for grafana if it doesn't exist, and empty service directory for grafana if it already contains a daemon file
RUN        mkdir -p /etc/service/grafana && rm -rf /etc/service/grafana/*

# Add daemon service for grafana-with-consul
ADD        start.sh /etc/service/grafana/run
